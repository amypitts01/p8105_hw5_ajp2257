...
We see here that when \(\mu=0\) we get the lowest amount of times the
null was rejected. That makes sense because we are running a hypothesis
test seeing if the sample is close to having a population mu of 0. This
is our alpha value or a false positive rate. When we increase the mu
value then the number of times the null was rejected increases. When mu
equal 5 and 6 then we are almost always rejecting the null hypothesis.
